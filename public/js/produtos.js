...
// Função para carregar os setores e preencher o dropdown "setor-select"
function loadSetores() {
  const setoresRef = db.collection("units").doc(unitId).collection("setores");
  // Se os documentos possuem o campo dataCadastro, use orderBy; senão, remova essa linha.
  setoresRef.orderBy("dataCadastro", "desc").onSnapshot(
    ...
    (error) => {
      ...
      document.getElementById(
        "setor-select"
      ).innerHTML = `<option value="">Erro ao carregar setores</option>`;
    }
  );
}
loadSetores();

// Se estiver editando (produtoId presente), carregar os dados do produto para edição
if (produtoId) {
  db.collection("units")
    .doc(unitId)
    .collection("produtos")
...
// Evento para envio do formulário para criar ou atualizar o produto
document
